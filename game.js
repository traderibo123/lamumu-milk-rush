...
// --- DOM ready guard (added) ---
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function(){ try{ if (typeof initOnce==='function') initOnce(); }catch(e){ console.error(e);} });
} else {
  try{ if (typeof initOnce==='function') initOnce(); }catch(e){ console.error(e); }
}
...
function initOnce(){
  ...
  // Safe event binding
  function safeOn(el, ev, fn){ if(el && el.addEventListener){ el.addEventListener(ev, fn); } }
  ...
  const ctx = el.stage.getContext('2d'); let W=el.stage.width, H=el.stage.height;
  function setupCanvasDPR(){
    const dpr = Math.max(1, Math.min(2, window.devicePixelRatio || 1));
    // keep logical size
    const displayWidth = el.stage.clientWidth || W;
    const aspect = H / W;
    const displayHeight = Math.round(displayWidth * aspect);
    // set canvas pixel size
    el.stage.width = Math.round(displayWidth * dpr);
    el.stage.height = Math.round(displayHeight * dpr);
    // set CSS size
    el.stage.style.width = displayWidth + "px";
    el.stage.style.height = displayHeight + "px";
  ...
  safeOn(el.btnStart,'click', startGame); el.btnHow.addEventListener('click',()=>setScreen('how'));
  ...
  function startGame(){
    ...
    updateShare(); el.state.textContent='—'; setScreen('game'); setupCanvasDPR(); requestAnimationFrame(loop);
  }
  function togglePause(){ if(!st.running) return; st.paused=!st.paused; el.btnPause.textContent=st.paused?'Devam':'Duraklat'; el.state.textContent=st.paused?'Duraklatıldı':'—'; }
  function spawn(){
  ...
  function updateShare(){ const text=encodeURIComponent(`Lamumu Milk Rush'ta ${st.score} puan yaptım! En iyi skor: ${st.best}.`);
    const url=encodeURIComponent('https://example.com'); const hashtags=encodeURIComponent('Lamumu,Common,Web3Game');
    const shareUrl=`https://twitter.com/intent/tweet?text=${text}&hashtags=${hashtags}&url=${url}`; el.share.setAttribute('href',shareUrl); el.shareOver.setAttribute('href',shareUrl); }
  ...
  // Fallback delegation for Start Game
  document.addEventListener('click', function(e){
    var t = e.target;
    if (!t) return;
    if (t.id === 'btn-start' || (t.closest && t.closest('#btn-start'))) {
